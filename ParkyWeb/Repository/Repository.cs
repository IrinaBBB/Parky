...
namespace ParkyWeb.Repository
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<bool> CreateAsync(string url, T objToCreate, string token = "")
        {
            ...
            {
                request.Content = 
                    new StringContent(JsonConvert.SerializeObject(objToCreate), Encoding.UTF8, "application/json");
            }
            else return false;
        ...
        public async Task<bool> UpdateAsync(string url, T objToUpdate, string token = "")
        {
            ...
            {
                request.Content =
                    new StringContent(JsonConvert.SerializeObject(objToUpdate), Encoding.UTF8, "application/json");
            }
            else return false;
